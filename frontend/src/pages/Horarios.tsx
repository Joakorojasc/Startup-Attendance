...
import { SCHEDULES, WORKERS } from '@/lib/mockData'

const DAY_LABELS = ['Dom', 'Lun', 'Mar', 'Mié', 'Jue', 'Vie', 'Sáb']
const ALL_DAYS = [0, 1, 2, 3, 4, 5, 6]

export function Horarios() {
  const [selected, setSelected] = useState<string>(SCHEDULES[0].id)

  const schedule = SCHEDULES.find((s) => s.id === selected)!
  const assignedWorkers = WORKERS.filter((w) => w.scheduleId === selected && w.status === 'active')
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded text-left transition-colors duration-100 ${
                    selected === s.id ? 'bg-accent/10 text-accent' : 'hover:bg-bg-hover text-text-secondary'
                  }`}
                >
        ...
        <div className="space-y-4">
          <div className="bg-bg-surface border border-border rounded-lg p-5">
            <div className="flex items-start justify-between mb-5">
              <div>
                <h2 className="text-base font-semibold text-text-primary">{schedule.name}</h2>
            ...
            <div className="grid grid-cols-3 gap-4 mb-6">
              <div className="bg-bg-elevated rounded p-3">
                ...
                <div className="text-lg font-semibold text-text-primary font-mono">{schedule.startTime}</div>
              </div>
              <div className="bg-bg-elevated rounded p-3">
                <div className="text-xs text-text-muted mb-1">Salida</div>
                <div className="text-lg font-semibold text-text-primary font-mono">{schedule.endTime}</div>
              </div>
              <div className="bg-bg-elevated rounded p-3">
            ...
            <div>
              ...
              <div className="flex gap-1.5">
                {ALL_DAYS.map((d) => {
                  const active = schedule.workDays.includes(d as 0)
                  return (
                    <div
                      key={d}
