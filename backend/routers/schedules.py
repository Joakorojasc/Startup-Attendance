...
async def list_schedules(db: Client = Depends(get_supabase)):
...
async def create_schedule(schedule: ScheduleCreate, db: Client = Depends(get_supabase)):
    result = db.table("schedules").insert(schedule.model_dump()).execute()
    return result.data[0]


@router.patch("/{schedule_id}", response_model=Schedule)
async def update_schedule(schedule_id: str, updates: ScheduleUpdate, db: Client = Depends(get_supabase)):
