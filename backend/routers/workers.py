...
def list_workers(status: str | None = None, db: Client = Depends(get_supabase)):
...
def get_worker(worker_id: str, db: Client = Depends(get_supabase)):
...
def create_worker(worker: WorkerCreate, db: Client = Depends(get_supabase)):
    result = db.table("workers").insert(worker.model_dump()).execute()
    return result.data[0]


@router.patch("/{worker_id}", response_model=Worker)
def update_worker(worker_id: str, updates: WorkerUpdate, db: Client = Depends(get_supabase)):
...
def deactivate_worker(worker_id: str, db: Client = Depends(get_supabase)):
